...
import ProductGallery from "./ProductDetailsCompnents/ProductGallery/ProductGallery";
import ProductInfo from "./ProductDetailsCompnents/ProductInfo/ProductInfo";
import DetailsSection from "./ProductDetailsCompnents/DetailsSection/DetailsSection";
import ReviewsSection from "./ProductDetailsCompnents/ReviewsSection/ReviewsSection";
import RelatedProducts from "./ProductDetailsCompnents/RelatedProducts/RelatedProducts";
...
export default function ProductDetails() {
  ...
  // const images = [Iphone, Iphone, Iphone, Iphone];
  const images = findProduct.media.gallery;

  const quickSpecs = [
    {
      label: "Screen size",
      value: findProduct.technical_specifications.display.size,
    },
    {
      label: "CPU",
      value: findProduct.technical_specifications.performance.cpu,
    },
    {
      label: "Number of Cores",
      value: findProduct.technical_specifications.performance.cores,
    },
    {
      label: "Main camera",
      value: findProduct.technical_specifications.camera.main,
    },
    {
      label: "Front-camera",
      value: findProduct.technical_specifications.camera.front,
    },
    {
      label: "Battery capacity",
      value: findProduct.technical_specifications.battery,
    },
  ];

  const screenSpecs = [
    { label: "Screen diagonal", value: findProduct.technical_specifications.display.size },
    {
      label: "The screen resolution",
      value: findProduct.technical_specifications.display.resolution,
    },
    {
      label: "The screen refresh rate",
      value: findProduct.technical_specifications.display.refresh_rate,
    },
    {
      label: "The pixel density",
      value: findProduct.technical_specifications.display.density,
    },
    {
      label: "Screen type",
      value: findProduct.technical_specifications.display.type,
    },
    {
      label: "Additionally",
      value: findProduct.technical_specifications.display.additional,
    },
  ];

  const cpuSpecs = [
    { label: "CPU", value: findProduct.technical_specifications.performance.cpu },
    { label: "Number of cores", value: findProduct.technical_specifications.performance.cores },
  ];
  ...
  return (
    <div className="min-h-screen bg-white bg-gray-50">
      <Breadcrumb />
      {/* <Breadcrumb items={["Home", "Journal", "Article Detail"]} /> */}

      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12 bg-white p-4">
          <ProductGallery images={images} productName={findProduct?.name} />

          <ProductInfo
            product={findProduct}
            name={findProduct?.name}
            price={findProduct?.pricing.current_price}
            originalPrice={findProduct?.pricing.original_price}
            colors={findProduct.colors}
            selectedColor={selectedColor}
            onColorChange={setSelectedColor}
            storageOptions={findProduct.storage_options}
        ...
        <DetailsSection
          description={findProduct.long_description}
          screenSpecs={screenSpecs}
          cpuSpecs={cpuSpecs}
        />

        <ReviewsSection
          overallRating={findProduct.reviews.average_rating}
          totalReviews={findProduct.reviews.total_count}
          ratingBreakdown={ratingBreakdown}
          reviews={findProduct.reviews.featured}
        />

        <RelatedProducts category={findProduct.category} brand={findProduct.brand} id={findProduct.id}/>
      </div>
    </div>
  );
}
